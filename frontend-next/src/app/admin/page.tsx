...
type Dietist = { id: number; username: string; name: string; password?: string }

export default function Admin() {
  const { token } = useContext(AuthContext)
  const [dietists, setDietists] = useState<Dietist[]>([])
  ...
  const createDietist = async (e: FormEvent) => {
    e.preventDefault()
    try {
      const res = await fetch('http://localhost:8080/api/v1/dietists', {
        method: 'POST',
  ...
  const deleteDietist = async (id: number) => {
  ...
  const updateDietist = async (d: Dietist) => {
